...
//GET request--get list of countries
...
//GET request--get single address
...
router.delete('/addresses/:id', verifyToken, async (req, res) => {
    try {
        let deletedAddress = await Address.delete({
            user: req.decoded._id,
            _id: req.params.id
        })
...
router.put('/addresses/set/default', verifyToken, async (req, res) => {
    try {
        ...
        if (updatedAddressUser) {
            res.json({
                success: true,
                message: "Successfully set this address to default"
            })
        }
    } catch (err) {
        res.status(500).json({
